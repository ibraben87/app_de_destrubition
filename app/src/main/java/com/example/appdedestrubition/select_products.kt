...
import com.example.appdedestrubition.model.Produit
...
class select_products : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        do {
            res?.next()
            val prix_catt: ResultSet? =con.cnx("SELECT * FROM `contient` WHERE `num_produit` = ${res!!.getInt("num_produit")} AND `nom_categorie` = 'categorie 3'")
            prix_catt!!.last()
